// A patient wayfinder: it knows the shortest route, but travels slowly and
// rests after every two or three steps.
const SIZE = 10;
const CELL = 64;
...
function setup() {
  document.title = "A Patient Wayfinder";
...
function newWorld() {
  ...
  world = {
    ...
    phase: "thinking",
    startedAt: now,
    nextStepAt: now + THINK_MS,
    animation: null,
    route: shortestPath(next.grid, next.start, next.goal).slice(1),
...
function updateAccessibility() {
  ...
  const { start, goal, current, phase, steps } = world;
  ...
  boardCanvas.elt.dataset.restUntil = String(Math.round(world.restUntil));
  boardCanvas.elt.dataset.elapsedMs = String(Math.round(performance.now() - world.startedAt));
}

function advance(now) {
  if (!world || world.phase === "arrived" || now < world.nextStepAt) return;
  if (world.phase === "thinking" || world.phase === "resting") world.phase = "walking";
  const next = world.route.shift();
  const from = { ...world.current };
  world.animation = { from, to: next, began: now, duration: STEP_MS };
  world.current = next;
  world.visits[next.y][next.x]++;
  world.steps++;
  if (same(next, world.goal)) {
    world.phase = "arrived";
  } else if (--world.stepsToRest === 0) {
    world.phase = "resting";
    world.restUntil = now + STEP_MS + REST_MS;
    world.nextStepAt = world.restUntil;
    world.stepsToRest = 2 + Math.floor(Math.random() * 2);
  } else {
    world.nextStepAt = now + STEP_MS;
  }
  updateAccessibility();
}
...
function drawGrid() {
  for (let y = 0; y < SIZE; y++) for (let x = 0; x < SIZE; x++) {
    ...
    if (!isWall && world.visits[y][x]) {
      fill(0, 0, 255, Math.min(world.visits[y][x] * 0.2, 0.9) * 255);
...
function drawWalker(now) {
  ...
  noStroke();
  fill(0, 0, 255);
  circle(x, y, 29 + Math.sin(now / 200) * 5);
...
if (typeof module !== "undefined") {
  module.exports = { shortestPath, makeWorld };
}
